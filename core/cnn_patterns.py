...
if __name__ == "__main__":
    ...
    # Mock ParamsManager for testing CNNPatterns standalone
    class MockParamsManager:
        def get_param(self, key, default=None):
            if key == 'current_cnn_architecture_key': return "default_simple"
            if key == 'cnn_architecture_configs':
                return {
                    "default_simple": {
    ...
    # Monkey patch ParamsManager for CNNPatterns instance for this test
    original_params_manager = None
    if 'core.params_manager' in sys.modules:
        original_params_manager = sys.modules['core.params_manager']
    from core import params_manager as pm_module
    pm_module.ParamsManager = MockParamsManager

    async def test_cnn_patterns_loading_and_prediction():
        logger.info("Starting CNNPatterns test...")
        # Ensure dummy model and scaler files exist for "default_simple" architecture
        # This part would typically be handled by the pre_trainer.py's main test run
        # For standalone test, we ensure they exist or create them.

        test_symbol = "ETH_EUR" # Example symbol
        test_timeframe = "1h"   # Example timeframe

        # Create dummy files if they don't exist
        for pattern_name in ['bullFlag', 'bearishEngulfing']:
            model_dir = os.path.join(CNNPatterns.MODELS_DIR, test_symbol, test_timeframe)
            os.makedirs(model_dir, exist_ok=True)

            model_path = os.path.join(model_dir, f"cnn_model_{pattern_name}_default_simple.pth")
            scaler_path = os.path.join(model_dir, f"scaler_params_{pattern_name}_default_simple.json")

            if not os.path.exists(model_path):
                dummy_arch = MockParamsManager().get_param('cnn_architecture_configs')['default_simple']
                # input_channels should match feature_names_in length in dummy_scaler_params
                # sequence_length also from dummy_scaler_params
                dummy_model = SimpleCNN(input_channels=12, num_classes=2, sequence_length=30, **dummy_arch)
                torch.save(dummy_model.state_dict(), model_path)
                logger.info(f"Created dummy model: {model_path}")

            if not os.path.exists(scaler_path):
                dummy_scaler_params = {
                    'min_': [0.0] * 12, 'scale_': [1.0] * 12, 'sequence_length': 30,
                    'feature_names_in_': ['open', 'high', 'low', 'close', 'volume', 'rsi', 'macd', 'macdsignal', 'macdhist', 'bb_lowerband', 'bb_middleband', 'bb_upperband']
                }
                with open(scaler_path, 'w') as f: json.dump(dummy_scaler_params, f)
                logger.info(f"Created dummy scaler: {scaler_path}")

        cnn_detector = CNNPatterns() # Will now use MockParamsManager

        # Create a mock dataframe
        mock_df = pd.DataFrame({
            'open': np.random.rand(50) * 100, 'high': np.random.rand(50) * 110,
            'low': np.random.rand(50) * 90, 'close': np.random.rand(50) * 100,
            'volume': np.random.rand(50) * 1000, 'rsi': np.random.rand(50) * 100,
            'macd': np.random.rand(50), 'macdsignal': np.random.rand(50), 'macdhist': np.random.rand(50),
            'bb_lowerband': np.random.rand(50)*95, 'bb_middleband':np.random.rand(50)*100, 'bb_upperband':np.random.rand(50)*105,
            'date': pd.date_range(end=datetime.now(), periods=50, freq='1h')
        })
        mock_df.set_index('date', inplace=True)

        logger.info("Testing predict_pattern_score for bullFlag...")
        score_bull = await cnn_detector.predict_pattern_score(mock_df, "ETH/EUR", "1h", "bullFlag")
        logger.info(f"bullFlag score: {score_bull}")
        assert 0.0 <= score_bull <= 1.0, "Bull flag score out of range."

        logger.info("Testing predict_pattern_score for bearishEngulfing...")
        score_bear = await cnn_detector.predict_pattern_score(mock_df, "ETH/EUR", "1h", "bearishEngulfing")
