...
if __name__ == "__main__":
    ...
    async def run_test_confidence_engine():
        ...
        initial_confidence = confidence_engine.get_confidence_score(test_token, test_strategy)
        initial_max_pct = confidence_engine.get_max_per_trade_pct(test_token, test_strategy)
        print(f"Initiële confidence: {initial_confidence:.3f}, Initiele MaxPerTradePct: {initial_max_pct:.3f}")

        print("\nSimuleer winnende trade (profit +2%, AI reported confidence 0.8)...")
        await confidence_engine.update_confidence(test_token, test_strategy, ai_reported_confidence=0.8, trade_result_pct=0.02)
        print(f"Confidence na win: {confidence_engine.get_confidence_score(test_token, test_strategy):.3f}, MaxPerTradePct: {confidence_engine.get_max_per_trade_pct(test_token, test_strategy):.3f}")

        print("\nSimuleer grote winnende trade (profit +5%, AI reported confidence 0.9)...")
        await confidence_engine.update_confidence(test_token, test_strategy, ai_reported_confidence=0.9, trade_result_pct=0.05)
        print(f"Confidence na grote win: {confidence_engine.get_confidence_score(test_token, test_strategy):.3f}, MaxPerTradePct: {confidence_engine.get_max_per_trade_pct(test_token, test_strategy):.3f}")


        print("\nSimuleer verliezende trade (profit -1%, AI reported confidence 0.7)...")
        await confidence_engine.update_confidence(test_token, test_strategy, ai_reported_confidence=0.7, trade_result_pct=-0.01)
        print(f"Confidence na verlies: {confidence_engine.get_confidence_score(test_token, test_strategy):.3f}, MaxPerTradePct: {confidence_engine.get_max_per_trade_pct(test_token, test_strategy):.3f}")

        print("\nSimuleer groot verliezende trade (profit -4%, AI reported confidence 0.8)...")
        await confidence_engine.update_confidence(test_token, test_strategy, ai_reported_confidence=0.8, trade_result_pct=-0.04)
        print(f"Confidence na groot verlies: {confidence_engine.get_confidence_score(test_token, test_strategy):.3f}, MaxPerTradePct: {confidence_engine.get_max_per_trade_pct(test_token, test_strategy):.3f}")


        print("\nSimuleer update alleen op AI-reflectie (AI reported confidence 0.9)...")
        await confidence_engine.update_confidence(test_token, test_strategy, ai_reported_confidence=0.9)
        print(f"Confidence na AI-reflectie: {confidence_engine.get_confidence_score(test_token, test_strategy):.3f}, MaxPerTradePct: {confidence_engine.get_max_per_trade_pct(test_token, test_strategy):.3f}")

        print("\nTest cooldown...")
        current_conf_before_cooldown = confidence_engine.get_confidence_score(test_token, test_strategy)
        await confidence_engine.update_confidence(test_token, test_strategy, ai_reported_confidence=0.2, trade_result_pct=-0.05)
        print(f"Confidence tijdens cooldown (zou gelijk moeten zijn aan {current_conf_before_cooldown:.3f}): {confidence_engine.get_confidence_score(test_token, test_strategy):.3f}")
